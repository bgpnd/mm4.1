    ...
    jQuery.fn.extend({
        submitAjax: function(options) {
            ...
            return this.each(function() {
                ...
                var element = $(this);
                var current = this;
                element.find('input[type="submit"], button[type="submit"]').on('click', function(event) {
                    ...
                    if (typeof trigger.loader != 'undefined') {
                        $('<span />').loader('show').prependTo(trigger.parent());
                    ...
                    jQuery.ajax({
                        ...
                        beforeSend  : function() {
                            ...
                            if (passed == false) {
                                ...
                                if (typeof trigger.loader != 'undefined') {
                                    trigger.parent().find('.loader').parent().remove();
                                }
                            }
                        ...
                        error       : function(xhr, textStatus, errorThrown) {
                            if(typeof options.onerror == 'function'){
                                options.onerror.call(this, xhr, element);
                            }
...
jQuery.fn.errorMessage = function(data) {
    if ((typeof data.error != "undefined" && data.error != "") || (typeof data.success != "undefined" && data.success == false)) {
        var text = (typeof data.error != "undefined" && data.error != "") ? data.error : data.message;
...
* @param  {String} action       'show' or 'hide', default to 'show'
* @param  {Array} options       [
*      dimension {String} 'small' or 'large' default to 'small',
*      backColor {String} 'light' or 'dark' default to 'light',
*      fullWidth {Boolean} default to false]
* @param  {Function} callback   function to call after call this function
*/
$.fn.loader = function(action, options, callback) {
    var defaults = {
            dimension: 'small',
            backColor: 'light',
            fullWidth: false
        ...
        fullWidth = (options.fullWidth === true) ? 'full' : '';

    return this.each(function() {
        // action = action || 'show';
        var $this = $(this),
            newHTML = '<div class="loader ' + options.dimension + ' ' + options.backColor + ' ' + fullWidth + '"></div>';
        if (action == 'show') {
            if ($this.parent().hasClass('inline-loader')) {
...
* @requirement : jquery.infinitescroll.min.js
* @author : Riza Arsyi
* @elements :
*     selector              : add new class on your content, eg. <ul selector>
*     item_selector         : add new class on your sub content, eg. <li item-selector>
*     navigation selector   : navigation selector is used to call request new pages on controller and return as ajax
*                             just add this script after selector element
*                             eg. <a id="next" href="<?php echo absUrl('/photos/home/index'); ?>" data-ajax="<?php echo $pages->params['total']; ?>">next page</a>
*                             attribute data-ajax is contain total_pages which is needed to set as "maxPage" on infinitescroll options.
* @intialization : initialize script on each $('.selector').infiniteScroll();
* more detail you can visit https://github.com/paulirish/infinite-scroll
*/
$.fn.infiniteScroll = function(options) {
    var defaults = {
        navSelector     : "#next:last",
        nextSelector    : "a#next:last",
        itemSelector    : this.selector + " .item-selector",
        finishedMsg     : "<em class='clearfix clear'>That's it</em>",
        totalPage       : $('a#next').attr('data-ajax'),
    },
    options = $.extend(defaults, options);
    ...
    this.infinitescroll({
        loading : {
            finished: undefined,
            finishedMsg     : "<em class='clearfix clear'>That's it</em>",
            img: "/themes/beautiplan-opalz/desktop/assets/images/spinner.gif",
            msg: null,
            msgText: "<em class='clearfix clear'>Loading next posts...</em>",
            selector: null,
            speed: 'fast',
            start: undefined
        },
        ...
        donetext  : options.finishedMsg,
        maxPage      : options.totalPage,
        path: function(index) {
            return $('#next').attr('href') + '?page=' + index;
        }
    }, function(json, opts) {
        $(this).append(json);
